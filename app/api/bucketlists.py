...
def post_bucketlist(user):
    ...
    db.session.commit()
    reponse_object = {"status": "Success", "bucketlist": blist.to_json()}
...
def get_bucketlist(user):
    ...
    if limit:
        ...
        bucketlists = BucketLists.query.filter(
            BucketLists.name.ilike("%" + q + "%"), BucketLists.owner == user
        ).all()
        return jsonify(
            {"search_results": [bucketlist.to_json() for bucketlist in bucketlists]}
        )
...
@bucketlist_blueprint.route(
    "/<int:bucketlist_id>/items/<int:item_id>", methods=["DELETE"]
)
@login_required
